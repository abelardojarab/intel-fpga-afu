...
wire incr_pulse = (byte64 & state == SOP &  ena & ~sleep) | (~byte64 & state == EOP &  ena & ~sleep)  ;
    ...
    always @(posedge clk) begin
		...
		else if(packet_length_cfg==start_addr&start_addr<=14'd68&state==IDLE&ena&nextpacket) packet_length_cfg <= packet_length_cfg+1;
		else if(incr_pulse) packet_length_cfg <= packet_length_cfg+1;
        ...
        case (state)
            IDLE :  begin
				...
				if (sleep) state_next = IDLE;
					...
					byte64_next = (packet_length_cfg <=14'd64+14'd4);
					byte128_next = (packet_length_cfg<=14'd128+14'd4)& (packet_length_cfg>14'd64+14'd4);	
