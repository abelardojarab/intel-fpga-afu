...
set split_qsys_output_name [split ex_100g_altera_xcvr_fpll_s10_htile_180_5guwkiq "_"]
...
source "${script_dir}/${top_inst_name}_ip_parameters_5guwkiq.tcl"
source "${script_dir}/alt_xcvr_fpll_helper_functions_5guwkiq.tcl"
...
# -                                                              - #
# ---------------------------------------------------------------- #
fpll_initialize_db_5guwkiq fpll_db_5guwkiq

# ---------------------------------------------------------------- #
# --- Set all the instances of this core                       --- #
# ---------------------------------------------------------------- #
set alt_xcvr_fpll_s10_instances [ dict keys $fpll_db_5guwkiq ]
...
foreach inst $alt_xcvr_fpll_s10_instances {
  ...
  dict set alt_xcvr_fpll_s10_pins $inst [dict get $fpll_db_5guwkiq $inst]
  ...
  for {set i 0} {$i < $profile_cnt} {incr i} {
    ...
    if {[dict exists $alt_xcvr_fpll_s10_pins $inst fpll_refclk_port]} {
      ...
      post_message -type info "IP SDC: The fPLL IP reference clock frequency is set to $refclk_freq"
      ...
      dict for {mode mode_clks} $profile_clocks {
        ...
        set list_of_clk_names [fpll_prepare_to_create_clocks_5guwkiq $inst $mode $mode_clks $profile_cnt $i $alt_xcvr_fpll_s10_pins $multiply_factor_dict $divide_factor_dict $all_fpll_profile_clocks_names]
...
post_message -type info "IP SDC: End of fPLL IP SDC file!"
