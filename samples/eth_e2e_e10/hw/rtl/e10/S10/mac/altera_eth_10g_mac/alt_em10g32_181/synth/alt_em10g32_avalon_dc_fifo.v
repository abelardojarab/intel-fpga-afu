...
module alt_em10g32_avalon_dc_fifo(
    ...
    sampling_clk,
    sampling_clk_reset_n,

    latency_out_clk,
    latency_out_clk_reset_n,
    ...
    output     [ADDR_WIDTH : 0] out_fill_level;
    output reg [ADDR_WIDTH : 0] in_fill_level;

    input      [ADDR_WIDTH : 0] almost_empty_threshold;
    input      [ADDR_WIDTH : 0] almost_full_threshold;
    ...
    input sampling_clk;
    input sampling_clk_reset_n;

    input latency_out_clk;
    input latency_out_clk_reset_n;

    output [ADDR_WIDTH+10:0] latency_out;
    ...
    alt_em10g32_dcfifo_synchronizer_bundle #(.WIDTH(ADDR_WIDTH+1), .DEPTH(WR_SYNC_DEPTH)) 
      write_crosser (
        .clk(out_clk),
        .reset_n(out_reset_n),
        .din(in_wr_ptr_gray),
    ...
    alt_em10g32_dcfifo_synchronizer_bundle #(.WIDTH(ADDR_WIDTH+1), .DEPTH(RD_SYNC_DEPTH)) 
      read_crosser (
        .clk(in_clk),
        .reset_n(in_reset_n),
        .din(out_rd_ptr_gray),
    ...
    // Latency measurement
    // ---------------------------------------------------------------------
    alt_em10g32_avalon_dc_fifo_lat_calc #(
        .ADDR_WIDTH(ADDR_WIDTH+1),
        .SYNC_DEPTH(WR_SYNC_DEPTH > RD_SYNC_DEPTH ? WR_SYNC_DEPTH : RD_SYNC_DEPTH)
    ) latency_calc (
        .wr_clk                     (in_clk),
        .reset_wr_clk_n             (in_reset_n),
        
        .rd_clk                     (out_clk),
        .reset_rd_clk_n             (out_reset_n),
        
        .sampling_clk               (sampling_clk),
        .reset_sampling_clk_n       (sampling_clk_reset_n),
        
        .latency_out_clk            (latency_out_clk),
        .reset_latency_out_clk_n    (latency_out_clk_reset_n),
        
        .wr_ptr                     (in_wr_ptr_gray),
        .rd_ptr                     (out_rd_ptr_gray),
