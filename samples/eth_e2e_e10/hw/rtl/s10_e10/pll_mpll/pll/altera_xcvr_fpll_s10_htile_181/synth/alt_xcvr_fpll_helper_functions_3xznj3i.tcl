...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc fpll_initialize_db_3xznj3i { fpll_db } {
  ...
  msg_vdebug "IP SDC: Initializing S10 fPLL database for CORE $::GLOBAL_corename"
  ...
  fpll_get_pins_3xznj3i $instance_name $all_pins
...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc fpll_get_pins_3xznj3i { instance all_pins } {
  ...
  set fpll_c0_div_reg_list   [fpll_get_fpll_counter_div_reg_node_3xznj3i]
  set fpll_refclk_port_list  [fpll_get_refclk_port_3xznj3i $instance $fpll_c0_div_reg_list]
...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc fpll_get_fpll_counter_div_reg_node_3xznj3i { } {
  global ::fpll_sdc_debug
...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc fpll_get_refclk_port_3xznj3i { instance fpll_c0_div_reg_list } {
  global ::fpll_sdc_debug
  ...
  if {[llength $fpll_c0_div_reg_list] > 0} {
    ...
    set no_inst_fpll_c0_div_reg_list [list]
    foreach fpll_c0_div_reg $fpll_c0_div_reg_list {
      # Remove the instance name from the clock source node due to auto promotion in SDC_ENTITY
      lappend no_inst_fpll_c0_div_reg_list [string replace $fpll_c0_div_reg 0 [string length $instance]]
    }

    # Grab all the fanins to the fpll_c?_div.reg nodes
    set fanin_col [get_fanins -clock -stop_at_clocks $no_inst_fpll_c0_div_reg_list]
...
# ---                                                                          --- #
# -------------------------------------------------------------------------------- #
proc fpll_prepare_to_create_clocks_3xznj3i { instance mode mode_clks profile_cnt profile alt_xcvr_fpll_s10_pins multiply_factor_dict divide_factor_dict all_profile_clocks_names } {
  global ::fpll_sdc_debug
    ...
    if { $mode == "refclk" } {
      ...
      if { [dict exists $alt_xcvr_fpll_s10_pins $instance $clk_group] } {
        ...
        if { [llength $clk_pins] > 0 } { # Check to see if the corresponding pins exists 
          ...
          lappend list_of_clk_names [fpll_create_clocks_3xznj3i $instance $clk_group $clk_pins $profile_cnt $profile $source_node $master_clock $multiply_factor $divide_factor]
        }
      } else {
        if {$fpll_sdc_debug == 1} {
...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc fpll_create_clocks_3xznj3i { instance clk_group clk_list profile_cnt profile source_node master_clock multiply_factor divide_factor } {
  global ::fpll_sdc_debug
