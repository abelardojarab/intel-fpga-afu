...
# Have a nogui switch for testing that throws errors instead of popping the UI
...
# Build the map - inputs can be command line, or auto-discovered
# On conversion, if the map is 1:1, that's ok
# If the map is not 1:1, don't covert. Post messages of problems, or pop
# a dialog
# Command line input or GUI disambiguates 1:many map entries
...
proc main { args } {
  ...
  switch -exact -- $opts(mode) {
    "build" {
      ...
      if { 0 != ([llength $args] % 2) } {
        post_message -type error "Specify <placeholder> <replacement instance> pairs"
        return
  ...
  switch -exact -- $opts(mode) {
    "build" {
      ...
      if { 0 < [llength $missing_cores] } {
        post_message -type error "The Signal Tap file could not be generated\
      ...
      dict for { placeholder new_ip_core_instance } $actual_replacements {
        ...
        if { 0 < [llength $foo] } {
          post_message -type warning "Some node names in the Signal Tap file\
...
#    bind ${cncb}.e <1> [namespace code [list paste_in %W]]
  ...
  proc assemble { args } {
    ...
    set stp_dialog [Dialog .stp_dialog -modal local -side bottom -anchor e \
      -title "Generate Signal Tap File" -cancel 1 -transient yes -parent .]
    ...
    set ipfl [label $ipf.l -text "The Signal Tap instance you generate\
...
#    $core_frame config -bg yellow
    ...
    set ofl [label $f.ofl -text "Signal Tap File:"]
  ...
  proc assemble_picker_only { args } {
    ...
    set stp_dialog [Dialog .stp_dialog -modal local -side bottom -anchor e \
      -title "Choose IP Core Instances for Signal Tap File" -default 0 \
...
#    $core_frame config -bg yellow
  ...
  proc pop_save { args } {
    variable stp_dialog_vars  
    set result [tk_getSaveFile -filetypes {{"Signal Tap Files" {".stp" ".STP"}}} \
