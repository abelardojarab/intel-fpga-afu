  ...
  switch -exact -- $opts(mode) {
    "build" {
      ...
      if { 0 < [llength $missing_cores] } {
        post_message -type error "The SignalTap file could not be generated\
      ...
      dict for { placeholder new_ip_core_instance } $actual_replacements {
        ...
        if { 0 < [llength $foo] } {
          post_message -type warning "Some node names in the SignalTap file\
...
################################################################################
# wire, node, net, clock
#      post_message "Checking that all names in SignalTap file exist in design"
#      set missing_names [check_names -dom $dom]
#      if { 0 < [llength $missing_names] } {
#        post_message -type error "The SignalTap file has names that don't exist\
...
#    bind ${cncb}.e <1> [namespace code [list paste_in %W]]
  ...
  proc assemble { args } {
    ...
    set stp_dialog [Dialog .stp_dialog -modal local -side bottom -anchor e \
      -title "Generate SignalTap File" -cancel 1 -transient yes -parent .]
    ...
    set ipfl [label $ipf.l -text "The SignalTap instance you generate\
...
#    $core_frame config -bg yellow
    ...
    set ofl [label $f.ofl -text "SignalTap File:"]
  ...
  proc assemble_picker_only { args } {
    ...
    set stp_dialog [Dialog .stp_dialog -modal local -side bottom -anchor e \
      -title "Choose IP Core Instances for SignalTap File" -default 0 \
...
#    $core_frame config -bg yellow
  ...
  proc pop_save { args } {
    variable stp_dialog_vars  
    set result [tk_getSaveFile -filetypes {{"SignalTap Files" {".stp" ".STP"}}} \
