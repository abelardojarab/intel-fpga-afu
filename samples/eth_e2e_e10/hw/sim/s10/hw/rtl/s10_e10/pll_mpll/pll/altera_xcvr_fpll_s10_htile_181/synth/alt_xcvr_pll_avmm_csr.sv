...
always@(posedge avmm_clk) begin
  if(~avmm_read) begin
    ...
    case(avmm_address)
      ...
      S10_XR_ADDR_PLL_MCGB_EN:avmm_readdata <= rd_mcgb_en;

      S10_XR_ADDR_GP_PLL_LOCK:avmm_readdata <= rd_ctrl_pll_lock;

      S10_XR_ADDR_GP_PLL_RST: avmm_readdata <= rd_pll_reset;
...
generate if(dbg_stat_soft_logic_enable == 1) begin: en_stat_reg
    ...
    assign rd_ctrl_pll_lock[S10_XR_OFFSET_GP_LOCK]      = pll_locked_sync;
    assign rd_ctrl_pll_lock[S10_XR_OFFSET_GP_CAL_BUSY]  = pll_cal_busy_sync;
    assign rd_ctrl_pll_lock[S10_XR_OFFSET_GP_AVMM_BUSY] = r_avmm_busy;
    assign rd_ctrl_pll_lock[S10_XR_OFFSET_LOCK_UNUSED+:S10_XR_LOCK_UNUSED_LEN] = {S10_XR_LOCK_UNUSED_LEN{1'b0}};
...
generate if(dbg_ctrl_soft_logic_enable == 1) begin: en_ctrl_reg
    ...
    assign rd_pll_reset[S10_XR_OFFSET_PLL_RST]        = r_pll_reset;
    assign rd_pll_reset[S10_XR_OFFSET_PLL_RST_OVR]    = r_pll_reset_override;
    assign rd_pll_reset[S10_XR_OFFSET_PLL_RST_UNUSED+:S10_XR_PLL_RST_UNUSED_LEN] = {S10_XR_PLL_RST_UNUSED_LEN{1'b0}};

    // assign the output control signal to the pll
    assign csr_pll_powerdown = (rd_pll_reset[S10_XR_OFFSET_PLL_RST_OVR]) ? rd_pll_reset[S10_XR_OFFSET_PLL_RST] : pll_powerdown;
    ...
    always@(posedge avmm_clk or posedge avmm_reset) begin
      ...
      end else if(avmm_write && avmm_address == S10_XR_ADDR_GP_PLL_RST) begin
        r_pll_reset           <= avmm_writedata[S10_XR_OFFSET_PLL_RST];
        r_pll_reset_override  <= avmm_writedata[S10_XR_OFFSET_PLL_RST_OVR];
      end
    end
  end else begin
